...
class Gutenberg
{
	...
	{
		if (wdpln_theme()->debug && is_user_logged_in()) {
			$this->min = true;
		}
		...
		if (file_exists(get_template_directory() . '../../static/fonts/fonts-woff2.css')) {
			$this->admin_font_path = get_template_directory() . '../../static/fonts/fonts-woff2.css';
			$this->admin_font_url = get_template_directory_uri() . '../../static/fonts/fonts-woff2.css';
		}
	}
	...
	{
		$settings = wdpln_theme()->getSettings();
	...
	public function enqueueBlockAssets()
	{
		if ($this->js) {
			...
			$script_asset = file_exists($script_asset_path) ? require($script_asset_path) : ['dependencies' => [], 'version' => wdpln_theme()->version];
			wp_enqueue_script(
				wdpln_theme()->prefix . '-gutenberg-script',
				$this->js,
		...
		if ($this->admin_font_url) {
			wp_enqueue_style(wdpln_theme()->prefix . '-gutenberg-font', $this->admin_font_url, [], filemtime($this->admin_font_path));
		}
	}
	...
	{
		wp_set_script_translations(wdpln_theme()->prefix . '-gutenberg-script', 'sht', get_template_directory() . '/languages');
	}
	...
	public function getLazyImage($post_id, $size, $figure_class, $image_class)
	{
		if ($this->isContextEdit()) {
			...
			$featured_image = wdpln_theme()->Package->Lazysizes->getLazyImage(get_post_thumbnail_id($post_id), $size, $figure_class, $image_class);
		}
